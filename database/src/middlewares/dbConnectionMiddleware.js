...
let {host, port, databaseName, admin, password} = config.mongo;

const connections = {
	admin: mongoose.createConnection(`mongodb://${admin}:${password}@${host}:${port}/${databaseName}`, { useNewUrlParser: true, useCreateIndex: true })
};
...
module.exports = async function (req, res, next) {
	try {
		...
		if (req.path === "/login") {
			...
			if (!connections[name]) {
				connections[name] = await mongoose.createConnection(`mongodb://${name}:${password}@${host}:${port}/${databaseName}`, { useNewUrlParser: true })
			}
